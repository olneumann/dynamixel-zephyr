...
int     portHandler         (const char *port_name) { return portHandlerZephyr(port_name); }

uint8_t openPort            (int port_num) { return openPortZephyr(port_num); }
void    closePort           (int port_num) { closePortZephyr(port_num); }
void    clearPort           (int port_num) { clearPortZephyr(port_num); }

void    setPortName         (int port_num, const char *port_name) { setPortNameZephyr(port_num, port_name); }
char   *getPortName         (int port_num) { return getPortNameZephyr(port_num); }

uint8_t setBaudRate         (int port_num, const int baudrate) { return setBaudRateZephyr(port_num, baudrate); }
int     getBaudRate         (int port_num) { return getBaudRateZephyr(port_num); }

int     readPort            (int port_num, uint8_t *packet, int length) { return readPortZephyr(port_num, packet, length); }
int     writePort           (int port_num, uint8_t *packet, int length) { return writePortZephyr(port_num, packet, length); }

void    setPacketTimeout    (int port_num, uint16_t packet_length) { setPacketTimeoutZephyr(port_num, packet_length); }
void    setPacketTimeoutMSec(int port_num, double msec) { setPacketTimeoutMSecZephyr(port_num, msec); }
uint8_t isPacketTimeout     (int port_num) { return isPacketTimeoutZephyr(port_num); }
#endif
